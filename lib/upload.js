...
function Upload(name, file, config, random) {
  if (!(this instanceof Upload)) {
    return new Upload(name, file, config, random);
  }

  this.name = sanitize(name).replace(/ /g, "_");
  this.file = file;
  this.config = config;
  this.random = random;
...
function upload(stream, headers, config, random) {
  ...
  function onFile(field, file, name) {
    var upload = Upload(name, file, config, random);
    uploads.push(upload);
